...
func GetTransactionByHashFromDB(ctx context.Context, odr OdrBackend, hash common.Hash) (*types.Transaction, common.Hash, uint64, uint64) {
	db := odr.Database()
...
func GetBlockFromDB(ctx context.Context, odr OdrBackend, hash common.Hash, number uint64) *types.Block {
	// Retrieve the block header and body contents
	header, err := GetHeaderByNumber(ctx, odr, number)
	if err != nil {
		log.Error(errNoHeader.Error())
		return nil
	}
	db := odr.Database()
	body := rawdb.ReadBody(db, hash, number)
